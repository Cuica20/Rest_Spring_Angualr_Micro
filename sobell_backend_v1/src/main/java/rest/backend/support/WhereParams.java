...
public class WhereParams {
    ...
    public String filter(String sqlFilter, Object paramValue) {
        if (paramValue == null || !StringUtility.hasText(String.valueOf(paramValue))) return "";
        String paramName = extractParamName(sqlFilter);
        params.put(paramName, paramValue);
        return sqlFilter;
    }
    ...
    public String replaceForDebug(String sql) {
        ...
        for (String paramName : params.keySet()) {
            ...
            buf = StringUtility.replace(buf, ":"+paramName, paramVal);
        }
        return buf.toString();
    }
    public String filterIsNull(String sqlFilterNotNull, String sqlFilterNull, Object paramValue) {
        if (paramValue == null || !StringUtility.hasText(String.valueOf(paramValue))) {
            try {
    ...
    public String filterIn(String sqlFilter, Collection paramsList, String enclosing) {
        ...
        String paramName = extractParamName(sqlFilter);
        sqlFilter = StringUtility.replace(sqlFilter, ":" + paramName, StringUtility.concat(paramsList, ",", enclosing));
        return sqlFilter;
    }
    ...
    private String extractParamName(String sqlFilter) {
        ...
        int idx2 = StringUtility.firstIdx(sqlFilter, idx, " ", ")", "\n", "\t");
